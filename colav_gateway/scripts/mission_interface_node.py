...
class MissionInterfaceNode(Node):
    ...
    def __init__(
        ...
        self._mission_request_address = (
            self.get_parameter("mission_request_host")
            .get_parameter_value()
            .string_value,
            self.get_parameter("mission_request_port")
            .get_parameter_value()
            .integer_value,
        )
        self._mission_response_address = (
            self.get_parameter("mission_response_host")
            .get_parameter_value()
            .string_value,
            self.get_parameter("mission_response_port")
            .get_parameter_value()
            .integer_value,
        )
        self._controller_feedback_address = (
            self.get_parameter("controller_feedback_host")
            .get_parameter_value()
            .string_value,
            self.get_parameter("controller_feedback_port")
            .get_parameter_value()
            .integer_value,
        )
    ...
    def listen_for_mission_request(self, action_srv_timeout: float = 5.0):
        ...
        self.get_logger().info(
            f"Checking /colav_gateway/execute_mission action srv is active before listening for mission request..."
        )
        # need to create an action client.
        execute_mission_cli = ActionClient(
            self, MissionExecutor, "/colav_gateway/execute_mission"
        )
        if not execute_mission_cli.wait_for_server(timeout_sec=action_srv_timeout):
            raise Exception(
                f"/execute_mision action srv not online therefore stopping node."
            )

        self.get_logger().info(
            f"/exeucte_mission action active, Listening for mission requests!"
        )
        ...
        while True:
            try:
                ...
                self.get_logger().info(
                    f"Mission Request protobuf received from {client_address}"
                )
                mission_request = ProtoToROSUtils.parse_mission_request(data)
                if mission_request:
                    self.execute_mission(
                        execute_mission_cli, mission_request=mission_request
                    )
            except TimeoutError:
                continue
                ...
                self.get_logger().warning(
                    f"mission_interface_node::listen_for_mission_request {e}"
                )
                continue
    ...
    def execute_mission(
        self, execute_mission_cli: ActionClient, mission_request: RosMissionRequest
    ):
        req = MissionExecutor.Goal()
        req.mission_request = mission_request
        send_goal_future = execute_mission_cli.send_goal_async(
            goal=req, feedback_callback=self.ctrl_feedback_callback
        )
        send_goal_future.add_done_callback(self._ctrl_goal_response_callback)

    def ctrl_feedback_callback(
        self, feedback: MissionExecutor.Feedback, timeout: float = 5.0
    ):

        try:
            self.get_logger().info(f"Feedback received: {feedback}")
            controller_feedback = bytes(
                feedback.serialised_protobuf_controller_feedback.data
            )
            sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            sock.sendto(controller_feedback, self._controller_feedback_address)
            self.get_logger(
                f"Controller feedback sent to {self._controller_feedback_address}"
            )
        except Exception as e:
